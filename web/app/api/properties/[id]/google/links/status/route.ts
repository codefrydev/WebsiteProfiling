...
export const GET: ApiRouteHandlerWithParams<{ id: string }> = async (
  ...
  return new Promise<Response>((resolve) => {
    ...
    const proc = spawn(
      pythonExe,
      ['-m', 'src', 'gsc-links-import', '--status', '--property-id', String(propertyId)],
    ...
    proc.on('close', (code: number | null) => {
      ...
      try {
        ...
        resolve(
          NextResponse.json({ error: raw || 'Status check failed', exitCode: code }, { status: 400 }),
        );
      } catch {
        resolve(
          NextResponse.json(
            { error: raw || 'Status check failed', exitCode: code },
