...
function resolveImageSrc(raw: string | null | undefined, pageUrl: string): string | null {
  let s = (raw || '').trim();
  if (!s) return null;
...
function collectFromLinks(links: ReportLink[] | undefined): GalleryImageItem[] {
  const map = new Map<string, GalleryImageItem>();
  const add = (rawSrc: string | null | undefined, pageUrl: string, kind: GalleryKind) => {
...
function GalleryTile({
  ...
  const kl = vg.kindLabels as Record<string, string>;
  const sj = strings.common;
  const [broken, setBroken] = useState(false);
  const primary = item.refs[0];
  const kinds = [...new Set(item.refs.map((r: { kind: string }) => r.kind))] as string[];
...
function getColumnCount(width: number, density: GalleryDensity): number {
