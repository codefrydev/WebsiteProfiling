...
import { Key, Search, AlertCircle, Download, Settings2 } from 'lucide-react';
...
const TAB_IDS = ['overview', 'all', 'questions', 'quickwins', 'lostclicks', 'opportunities', 'cannib', 'bypage'];
const EMPTY_ROWS = [];
const EMPTY_HISTORY = {};

export default function KeywordsExplorer({ onOpenIntegrations }) {
  const { data, startUrlByRunId, selectedReportId } = useReport();
  const ke = strings.views.keywordsExplorer;
  const kwData = data?.keywords;
  const rows = Array.isArray(kwData?.rows) ? kwData.rows : EMPTY_ROWS;

  const startUrl = startUrlByRunId?.get?.(selectedReportId) || data?.google?.gsc?.site_url || '';
  const brandName = (kwData?.brand_name || deriveBrandFromUrl(startUrl) || '').trim();
  ...
  const tabLabels = ke.tabs;

  const quickWinCount = useMemo(
    () =>
      rows.filter((r) => {
        const pos = parseFloat(r.gsc_position || 0);
  ...
  const cannibItems = kwData?.cannibalisation || [];
  ...
  const headerMeta = kwData?.fetched_at ? (
    <span>
      {' '}
      &middot; {format(ke.subtitleWithDate, { date: new Date(kwData.fetched_at).toLocaleString() })}
    </span>
  ) : null;
    ...
    return (
      <PageLayout>
        ...
        <div className="max-w-md mx-auto text-center py-12">
          ...
          <p className="text-xs text-muted-foreground font-mono">{ke.emptyHint}</p>
        </div>
    ...
    <PageLayout>
      ...
      {kwData.fetched_at && (
        <p className="text-xs text-muted-foreground">
          {format(ke.footer.lastEnrichment, { date: new Date(kwData.fetched_at).toLocaleString() })}
          {kwData.suggest_count > 0 && format(ke.footer.suggestCount, { n: kwData.suggest_count })}
          {kwData.cannibalisation_count > 0 && format(ke.footer.cannibCount, { n: kwData.cannibalisation_count })}
        </p>
      )}
    </PageLayout>
  );
