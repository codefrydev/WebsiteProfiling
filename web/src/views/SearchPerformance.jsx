...
const TABS = ['overview', 'queries', 'pages', 'opportunities', 'coverage'];
...
export default function SearchPerformance() {
  ...
  const pageColumns = useMemo(
    () => [
      ...
      },
      {
        ...
        render: (_, row) => (
          <a
            href={buildLinksInspectHref(row.page, searchParams)}
  ...
  const insights = useMemo(() => {
    ...
    if (urlJoin?.gsc_only > 0) {
      bullets.push(format(sp.insights.gscOnly, { count: urlJoin.gsc_only }));
    }
    return bullets;
  }, [gsc, opportunities, urlJoin, sp]);

  const tabLabels = sp.tabs;
  const coverageBadge = urlJoin?.gsc_only > 0 ? urlJoin.gsc_only : null;

  const headerMeta = google?.fetched_at ? (
    <span>
      {' '}
      &middot; {format(sp.fetchedAt, { date: new Date(google.fetched_at).toLocaleDateString() })}
    </span>
  ) : null;
            ...
            <div id="gsc-tab-coverage" role="tabpanel" aria-labelledby="gsc-tab-btn-coverage" className="space-y-6">
              ...
              {urlJoin ? (
                <>
                  ...
                  {urlJoin.lists?.gsc_only?.length > 0 || urlJoin.lists?.crawl_only?.length > 0 ? (
                    <UrlGapListsPanel
                      urlJoin={urlJoin}
