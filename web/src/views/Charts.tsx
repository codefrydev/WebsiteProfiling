...
const barValueLabelsPlugin = {
  ...
  afterDatasetsDraw(chart: Chart) {
    ...
    meta.data.forEach((bar, i) => {
      const el = bar as { x: number; y: number; width?: number; height?: number };
      const value = dataset.data[i];
      if (value == null) return;
      const label = Number(value).toLocaleString();
      const x = isHorizontal ? el.x + (el.width != null && el.width >= 0 ? 6 : -6) : el.x;
      const y = isHorizontal ? el.y : el.y - (el.height != null && el.height >= 0 ? 12 : -12);
...
type BubblePoint = { x: number; y: number; r: number; url: string };
type ScatterPoint = { x: number; y: number; url: string };

function barOptsY(ariaSummary?: string) {
      ...
      tooltip: {
        callbacks: {
          label: (ctx: TooltipItem<'bar'>) =>
            ` ${ctx.raw?.toLocaleString() ?? ctx.raw} ${ch.axisUrls}`,
        },
      },
    },
    scales: {
...
function barOpts(ariaSummary?: string) {
  const ch = strings.charts;
  return anyChartOptions({
    responsive: true,
    maintainAspectRatio: false,
...
export default function Charts({ searchQuery = '' }: ViewProps) {
  ...
  const mimeLabelsTop = mimeSorted.labels.slice(0, 12);
  const mimeValuesTop = mimeSorted.values.slice(0, 12);
    ...
    <PageLayout className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <Card padding="tight" className="print:break-inside-avoid">
          ...
          <div className="h-64" role="img" aria-label={mimeLabelsTop.length ? `${vc.ariaMime} ${mimeLabelsTop[0]}: ${mimeValuesTop[0]}.` : sj.noData}>
            {mimeLabelsTop.length > 0 ? (
              <Bar
                data={{
                  labels: mimeLabelsTop,
                  datasets: [{ data: mimeValuesTop, backgroundColor: palette(mimeLabelsTop.length) }],
                }}
                options={barOpts()}
                plugins={[barValueLabelsPlugin]}
              />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        {(() => {
          ...
          links.forEach((l) => { wordCountMap[l.url] = l.word_count || 0; });
          const bubbleData: BubblePoint[] = topPages
            .filter((p) => {
              if (!q) return true;
        ...
        {(() => {
          const links = data.links || [];
          const scatterData: ScatterPoint[] = links
            .filter((l) => (l.word_count ?? 0) > 0 && (l.response_time_ms ?? 0) > 0)
            .filter((l) => {
              if (!q) return true;
            ...
            .slice(0, 200)
            .map((l) => ({ x: l.word_count ?? 0, y: l.response_time_ms ?? 0, url: l.url }));
          return (
            <Card padding="tight" className="print:break-inside-avoid">
