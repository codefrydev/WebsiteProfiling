...
function barOptsVertical(yTitle: string, ariaDescription?: string) {
  ...
  return {
    ...
    plugins: {
      ...
      tooltip: { callbacks: { label: (ctx: TooltipItem<'bar'>) => ` ${format(o.tooltipCountBar, { count: Number(ctx.raw).toLocaleString() })}` } },
    },
    scales: {
...
function barOptsGrouped(yTitle: string) {
  const o = strings.views.overview;
  return {
    responsive: true,
    maintainAspectRatio: false,
    plugins: {
      ...
      tooltip: {
        callbacks: {
          label: (ctx: TooltipItem<'bar'>) => ` ${format(o.tooltipGroupedBar, { dataset: ctx.dataset.label, count: Number(ctx.raw).toLocaleString() })}`,
        },
      },
    },
    scales: {
...
function barOptsSocial() {
  ...
  return {
    ...
    plugins: {
      ...
      tooltip: {
        callbacks: {
          label: (ctx: TooltipItem<'bar'>) => ` ${format(o.socialTooltipHtml, { label: ctx.label, pct: Number(ctx.raw).toFixed(1) })}`,
        },
      },
    },
    scales: {
...
function sumObject(obj: Record<string, unknown> | null | undefined): number {
...
export default function Overview({ searchQuery = '' }: ViewProps) {
  ...
  return (
    <PageLayout className="space-y-8">
      ...
      {(data.ml_errors?.length ?? 0) > 0 && (
        <div className="rounded-xl border border-amber-500/40 bg-amber-100/60 dark:bg-amber-950/25 px-4 py-3">
          ...
          >
            ...
            {format(vo.mlErrors, { count: data.ml_errors?.length ?? 0, plural: (data.ml_errors?.length ?? 0) !== 1 ? 's' : '' })}
          </button>
          {mlErrOpen && (
      ...
      {(data.keyword_opportunities?.quick_wins?.length ?? 0) > 0 || (data.keyword_opportunities?.high_value?.length ?? 0) > 0 ? (
