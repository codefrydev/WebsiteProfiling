/**
 * LLM / AI enrichment settings — stored only in report.db (llm_config table).
...
export const LLM_CONFIG_SECTIONS = [
  {
    ...
    fields: [
      ...
      },
      {
        ...
        type: 'secret',
        defaultValue: '',
        help: 'Optional if set via OPENAI_API_KEY, GEMINI_API_KEY, or ANTHROPIC_API_KEY. Never written to pipeline-config.txt.',
      },
    ],
  },
  {
...
export function isLlmSecretKey(key) {
  return SECRET_KEYS.has(key);
}

export function getLlmFieldByKey(key) {
...
export function buildInitialLlmConfigState() {
  const out = {};
  for (const section of LLM_CONFIG_SECTIONS) {
...
export function maskLlmSecretForClient(key, value) {
