...
export default function PageAnalysisTab({ link }: PageAnalysisTabProps) {
  ...
  const lhCategoryChart = useMemo(() => {
    ...
    const categoryLabels = lhLabels as Record<string, string>;
              ...
              {['performance', 'accessibility', 'best-practices', 'seo'].map((cat) => {
                ...
                return (
                  <div key={cat} className="bg-brand-900 rounded-xl p-3 border border-default text-center">
                    <div className="text-xs text-muted-foreground capitalize mb-1">{categoryLabels[cat] || cat.replace('-', ' ')}</div>
              ...
              <div className="mt-6 space-y-2">
                ...
                <ul className="space-y-2">
                  {failingLighthouseAudits.map((a: LighthouseAuditRef) => (
                    <LhAuditExpandable key={a.id || a.title} audit={a} />
                  ))}
                </ul>
              </div>
            )}
        ...
        <div className="space-y-2">
          {p.resourceSections.map(({ key, label }: { key: keyof PageAnalysis; label: string }) => (
            <ResourceSection
              key={key}
              title={label}
              urls={pa[key] as string[] | undefined}
              pageUrl={link.url}
