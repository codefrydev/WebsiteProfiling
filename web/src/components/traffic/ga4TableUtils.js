...
export function filterLowEngagement(pages) {
...
export function buildEngagementBuckets(pages) {
  const buckets = [0, 0, 0, 0];
  for (const p of pages || []) {
    const rate = parseFloat(p.engagementRate);
...
export function buildPageExportColumns(tf) {
  return [
