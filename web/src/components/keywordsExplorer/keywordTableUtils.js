...
export function difficultyColor(kd) {
...
export function buildIntentCounts(rows) {
  const counts = { informational: 0, commercial: 0, transactional: 0, navigational: 0, other: 0 };
  for (const r of rows) {
    const intent = r.intent || 'other';
    if (counts[intent] != null) counts[intent] += 1;
    else counts.other += 1;
  }
  return counts;
}

export function buildSourceCounts(rows) {
  const counts = {};
  for (const r of rows) {
...
export function exportKeywordCsv(rows) {
  ...
  const lines = [
    ...
    ...rows.map((r) =>
      cols
        .map((c) => {
          if (c === 'gsc_ctr' && r[c] != null) return escape(formatGscCtr(r[c]));
          return escape(r[c]);
        })
        .join(','),
    ),
...
export function buildExportColumns(ke) {
  return [
...
export function isKeywordBrandRelevant(row, brandName) {
...
export function filterRowsByTab(rows, tabId, options = {}) {
