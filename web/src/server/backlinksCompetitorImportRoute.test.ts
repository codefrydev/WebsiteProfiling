...
describe('backlinks/competitor-import route proxy', () => {
  ...
  it('returns 403 for non-local', async () => {
    ...
    const res = await POST(remoteRequest('/api/backlinks/competitor-import', { method: 'POST' }));
    expect(res.status).toBe(403);
  });
  it('proxies POST to FastAPI', async () => {
    const { POST } = await import('../../app/api/backlinks/competitor-import/route');
    const req = localRequest('/api/backlinks/competitor-import', { method: 'POST', body: '{}' });
    const res = await POST(req);
    expect(res.status).toBe(200);
    expect(proxyMock).toHaveBeenCalledWith(req, '/api/backlinks/competitor-import');
  });
});
