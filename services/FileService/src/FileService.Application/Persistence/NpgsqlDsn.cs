...
/// <summary>
/// Converts a libpq connection URI (<c>postgres://user:pass@host:port/db?connect_timeout=3</c>) — the
/// form used by <c>DATABASE_URL</c> across the Python services — into an Npgsql keyword connection
/// string. Npgsql's parser does NOT accept the <c>postgres://</c> URI form and throws on it, so this
/// conversion is mandatory. A string that is already in keyword form is passed through unchanged.
/// (Copied from the Data service so FileService owns its own DB access.)
/// </summary>
public static class NpgsqlDsn
{
    public static string ToNpgsql(string raw)
    {
        ...
        if (!isUri)
        {
            return s; // already an Npgsql keyword connection string
        }
        ...
        // Translate the query params we understand; strip the rest (Npgsql throws on unknown keywords).
        foreach (var (key, value) in ParseQuery(uri.Query))
        {
            switch (key.ToLowerInvariant())
            {
                case "connect_timeout":
                    ...
                    {
                        b.Timeout = t; // Npgsql "Timeout" == connect timeout (seconds)
                    }
                    break;
                case "sslmode":
                ...
                case "application_name":
                    ...
                    break;
                // unknown params are intentionally dropped
            }
        }
